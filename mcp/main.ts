...
async function addTodoItem(title: string) {
  try {
    ...
    if (!response.ok) {
      console.error(
        `[addTodoItem] APIサーバーエラー: ${response.status} ${response.statusText}`
      );
      return null;
    }
...
async function updateTodoItem(id: number, completed: boolean) {
  try {
    ...
    if (!response.ok) {
      console.error(
        `[updateTodoItem] APIサーバーとの通信エラー: ${response.status} ${response.statusText}`
      );
      return null;
    }
    return true;
...
app.get('/sse', (c) => {
  ...
  return streamSSE(c, async (stream) => {
    try {
      ...
      console.log(
        `[SSE] Connected to MCP server: sessionId= ${transport.sessionId}`
      );
